...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const tenant = await prisma.tenant.findUnique({
      where: { id },
      include: {
        users: { where: { deletedAt: null }, select: { id: true, email: true, name: true, role: true, status: true, isActive: true } },
    ...
    return NextResponse.json({ success: true, tenant, data: { tenant } });
  } catch (error: any) {
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, tenant: updated, data: { tenant: updated } });
  } catch (error: any) {
