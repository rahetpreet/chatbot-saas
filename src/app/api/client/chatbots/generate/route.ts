...
const FLOW_SYSTEM_PROMPT = `You design conversational chatbot flows as directed graphs.
 ...
 */
function normalizeGeneratedGraph(raw: any): NormalizedGraph | null {
    ...
    if (generator) {
      try {
        const completion = await generator.complete({
          ...
          temperature: 0.4,
          user: `Business/company name: ${tenant?.name || "our company"}

Design a complete chatbot flow for this request:
"""
${String(prompt || templatePreset).slice(0, 4000)}
"""

Requirements:
- Greet the visitor and introduce the business by its real name.
- Offer a menu of the actual products, services or intents named in the request.
- Branch each menu option to its own path using sourceHandle.
- Collect the contact details the request asks for (name, email, phone, and anything else mentioned).
- Offer a path to a human handover.
- Confirm and close at the end of every path.
