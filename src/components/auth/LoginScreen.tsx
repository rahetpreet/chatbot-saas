...
export interface LoginBrand {
  /** Business name shown instead of the platform's. */
  name: string;
  logoUrl?: string | null;
}
 ...
 * The sign-in screen.
 *
 * Lives in a component rather than only in the page because a workspace with
 * its own domain signs in at that domain's root, and should see its own name
 * there rather than the platform's — otherwise the white-labelling stops at
 * the chat window.
 */
        ...
        <div className="text-center space-y-2">
          {brand?.logoUrl ? (
            <img
              src={brand.logoUrl}
              alt=""
              className="mx-auto h-12 w-12 rounded-2xl object-cover shadow-lg"
              // A broken logo URL must not leave an empty gap where the
              // identity should be.
              onError={(event) => {
          ...
          <h1 className="text-2xl font-black tracking-tight text-white">{brand?.name || "ChatFlow Platform"}</h1>
