...
function CampaignChatContainer({ tenantSlug, initialFlowId }: { tenantSlug: string; initialFlowId?: string }) {
  ...
  const handleSendMessage = async (inputObj: {
    ...
    try {
      ...
      // A short beat so the typing indicator reads as deliberate rather than a
      // flicker. 1.5s made every reply feel slow.
...
export function CampaignChatContent({ tenantSlug, initialFlowId }: { tenantSlug: string; initialFlowId?: string }) {
  return (
